You are an OpenCode automation engineer. Build a local-first, free-to-use, TypeScript/JavaScript‑native, **pnpm**‑based MCP for OpenCode, named **陳永仁係二五仔-內鬼-間諜-SpyFamily成員** (hereafter referred to as the *MCP*). The MCP must orchestrate the full software development lifecycle using:
...
- the MCP command (`陳永仁係二五仔-內鬼-間諜-SpyFamily成員`)
