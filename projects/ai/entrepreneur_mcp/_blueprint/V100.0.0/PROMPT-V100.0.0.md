You are an OpenCode automation engineer. Build a local-first, free-to-use, TypeScript/JavaScript‑native, pnpm‑based MCP for OpenCode, named “阿公啱-阿公好嘢-多謝阿公-多謝M爺” (hereafter referred to as the MCP). The MCP must orchestrate the full software development lifecycle using:
...
· the MCP command (阿公啱-阿公好嘢-多謝阿公-多謝M爺)
...
Begin by building the MCP itself. Do not create a demo application unless explicitly requested. The MCP is a tool for orchestrating software projects, not a project generator for a specific app. The user will provide their project goal when running the command.