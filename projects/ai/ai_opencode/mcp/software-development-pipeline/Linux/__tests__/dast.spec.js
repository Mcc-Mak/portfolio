...
describe('normalizeConfig', () => {
  ...
  it('validates scanTimeoutMs', () => {
    expect(normalizeConfig({ scanTimeoutMs: 120000 }).scanTimeoutMs).toBe(120000);
    expect(() => normalizeConfig({ scanTimeoutMs: 500 })).toThrow(RangeError);
  });

  it('applies the ZAP API defaults', () => {
    const c = normalizeConfig({});
    expect(c.zap.api.baseUrl).toBe('http://zap:8080');
    expect(c.zap.api.apiKey).toBe('');
    expect(c.scanTimeoutMs).toBe(300000);
  });

  it('preserves staging and zap overrides', () => {
    const c = normalizeConfig({
      stage: { enabled: true, command: 'node src/index.js' },
      zap: { image: 'custom/zap', api: { baseUrl: 'http://zap:8090', apiKey: 'secret' } }
    });
    expect(c.stage.command).toBe('node src/index.js');
    expect(c.zap.image).toBe('custom/zap');
    expect(c.zap.api.baseUrl).toBe('http://zap:8090');
    expect(c.zap.api.apiKey).toBe('secret');
    expect(DEFAULT_CONFIG.zap.baselineScript).toBe('zap-baseline.py');
  });
});

describe('detectEngine', () => {
  it('prefers a running ZAP API over CLI and Docker', () => {
    const engine = detectEngine({ apiAvailable: true, dockerAvailable: true, zapImagePresent: true, zapCliAvailable: true });
    expect(engine.available).toBe(true);
    expect(engine.mode).toBe('api');
    expect(engine.kind).toBe('free');
    expect(engine.onPrem).toBe(true);
  });

  it('prefers the ZAP CLI over Docker when both exist', () => {
    const engine = detectEngine({ apiAvailable: false, dockerAvailable: true, zapImagePresent: true, zapCliAvailable: true });
    expect(engine.available).toBe(true);
...
describe('parseZapJson', () => {
  ...
  it('parses the classic report shape with instances[] and riskcode', () => {
    const report = JSON.stringify({
      site: [
        {
          '@name': 'http://localhost:3000',
          alerts: [
            { pluginid: '40018', alert: 'SQL Injection', riskcode: '3', riskdesc: 'High (Medium)', cweid: '89', count: '2', instances: [{ uri: 'http://localhost:3000/login', evidence: 'x' }] },
            { pluginid: '10021', alert: 'Header', riskcode: '1', riskdesc: 'Low (Medium)', cweid: '-1', instances: [{ uri: 'http://localhost:3000/' }] }
          ]
        }
      ]
    });
    const findings = parseZapJson(report);
    expect(findings).toHaveLength(2);
    const sql = findings[0];
    expect(sql.id).toBe('40018');
    expect(sql.severity).toBe('high');
    expect(sql.cwe).toBe('CWE-89');
    expect(sql.uri).toContain('/login');
    expect(sql.evidence).toBe('x');
    expect(findings[1].cwe).toBeNull();
    expect(findings[1].severity).toBe('low');
  });

  it('parses the JSON API alerts[] shape with named risk', () => {
    const report = JSON.stringify({
      alerts: [
        { pluginId: '40018', name: 'SQL Injection', risk: 'High', url: 'http://localhost:3000/login', cweid: '89', confidence: 'Medium' },
        { pluginId: '0', name: 'Info', risk: 'Informational', url: 'http://localhost:3000/', cweid: '-1' }
      ]
    });
    const findings = parseZapJson(report);
    expect(findings).toHaveLength(2);
    expect(findings[0].severity).toBe('high');
    expect(findings[0].owasp).toContain('Injection');
    expect(findings[0].uri).toContain('/login');
