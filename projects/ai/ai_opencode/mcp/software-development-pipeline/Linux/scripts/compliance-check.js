...
const TEXT_EXTS = new Set(['.md', '.js', '.json', '.sh', '.txt', '.yaml', '.yml', '.html', '.xml']);
...
function scanTextFiles(projectDir) {
  ...
  const evidencePath = path.join(projectDir, 'metrics', 'compliance-evidence.md');
  if (fs.existsSync(evidencePath)) {
    roots.push({ relative: 'metrics/compliance-evidence.md', absolute: evidencePath });
  }

  for (const root of roots) {
    const stat = fs.statSync(root.absolute);
    if (stat.isFile()) {
      if (stat.size <= MAX_FILE_BYTES && TEXT_EXTS.has(path.extname(root.relative).toLowerCase())) {
        try {
          const text = fs.readFileSync(root.absolute, 'utf8').replace(/^\uFEFF/, '');
          entries.push({ file: root.relative, text });
        } catch (_) { /* skip */ }
      }
    } else {
