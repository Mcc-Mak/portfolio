...
| DAST | R19 | `npm run dast` (on-prem OWASP ZAP - running instance via JSON API at http://zap:8080, else Docker/CLI; warn+block if no free engine, recommends 2 best paid on-prem engines) |
| Mandatory gate | R10 | fail on any gate error |
| Audit trail | R11 | `logs/audit.log` |

For hardening guidance see [Security Hardening Guide](docs/05_Security_Compliance/sec-hardening.md).
