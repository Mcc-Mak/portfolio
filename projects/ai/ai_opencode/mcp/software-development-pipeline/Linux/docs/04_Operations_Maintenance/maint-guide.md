...
truth `/workplace/mcp/software-development-pipeline/Linux`. The deploy script
(`scripts/deploy-global.sh`, via `npm run deploy`) is non-destructive and
...
```mermaid
flowchart LR
    P[Project source<br/>scripts/, skills/, src/, docs/] -->|npm run deploy| D[deploy-global.sh]
...
| `scripts/*.sh` / `*.js` fix | Edit project source; run `npm run deploy`; smoke test |
