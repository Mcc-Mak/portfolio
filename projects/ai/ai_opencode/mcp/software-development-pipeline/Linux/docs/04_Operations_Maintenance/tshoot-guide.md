...
| Symptom | Cause | Fix |
| :--- | :--- | :--- |
| `opencode: command not found` | Global CLI not installed or npm global bin not on PATH | `npm install -g opencode-ai`; add `${HOME}/.npm-global/bin` (or `${HOME}/.local/bin`) to PATH |
| `scripts/*.sh` fails with permission denied | Shell script lacks the execute bit | `chmod +x scripts/*.sh` |
