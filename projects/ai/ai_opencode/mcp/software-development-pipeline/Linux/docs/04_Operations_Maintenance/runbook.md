...
cp -v "$HOME/.config/opencode/metrics/metrics.db" "$BACKUP_DIR/metrics.db.$(date +%Y%m%d)"
cp -v "$HOME/.config/opencode/logs/audit.log" "$BACKUP_DIR/audit.log.$(date +%Y%m%d)"
```
...
cp -v "$BACKUP_DIR/metrics.db.<date>" "$HOME/.config/opencode/metrics/metrics.db"
