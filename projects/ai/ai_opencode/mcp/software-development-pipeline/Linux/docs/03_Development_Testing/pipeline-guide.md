...
From the OpenCode TUI the same runs are `/pipeline noskip|skipdocs|skipsecurity`,
plus `/pipeline ssot`, which does NOT run the pipeline — it creates or updates
`opencode.project.md` (the SSOT) via a one-by-one TUI interview (see section 6).

All entry points run `scripts/opencode-pipeline.sh` via
`bash scripts/opencode-pipeline.sh`. The script infers the global toolchain
...
| 4 - Documentation | `doc-generation` | `specs/`, `src/`, `docs/` | `docs/*`, `docs/toctree.md`, `README.md`, root templates | any required doc file missing |
...
`opencode.project.md` is the SINGLE SOURCE OF TRUTH (SSOT). When it exists in
the project directory, its content is injected into the Phase 1, Phase 2, and
Phase 4 prompts so every artifact matches the project's domain (stack, data
sources, UI layout, forms). Without it, the pipeline runs generically. This is
the project-level customization hook.

Create or update it interactively with `/pipeline ssot`: it asks for the project
information ONE question at a time in the TUI, then writes
`opencode.project.md` following the canonical SSOT format (20 numbered sections:
identity, stack, quick start, repository layout, configuration, conventions,
architecture, schema, data sources, API, workflow, parameters, UI, status
matrix, calculations, products, deployment, security, troubleshooting,
glossary). If the file already exists, unchanged sections are preserved.

When the SSOT is present, `src/` is the **ROOT DIRECTORY of the codebase**.
Phase 2 follows the SSOT's Repository Layout section exactly (e.g.
`src/server.js`, `src/database/`, `src/frontend/`) rather than a flat `src/*.js`
dump, and Phases 1/4 reference source paths with `src/` as the code root.

All generated markdown embeds Mermaid/PlantUML/Graphviz diagrams per the skill
Diagram Policy. Mermaid syntax MUST follow the CURRENT release used by
https://mermaid.live (Mermaid v11.16.x). Both canonical and `-beta` keyword
forms are accepted. All diagrams are validated by `scripts/validate-mermaid.js`
(R10 gate).
