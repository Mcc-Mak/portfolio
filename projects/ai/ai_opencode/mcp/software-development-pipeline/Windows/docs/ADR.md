...
| ID | Title | Status | ADR |
| :--- | :--- | :--- | :--- |
| ADR-001 | PowerShell orchestrator | Accepted | below |
...
## ADR-001 - PowerShell orchestrator

**Status**: Accepted

**Context**: The pipeline must run on Windows without WSL, cloud, or a
...
**Decision**: Implement the orchestrator as `scripts/opencode-pipeline.ps1`
using PowerShell. It sequences the four phases, runs the mandatory gates, and
appends one audit line per phase/gate.

**Consequences**:
- Native Windows execution with no extra runtime.
- PowerShell is the only shell available to all phases on the target host.
- Logic is not portable to POSIX hosts without a rewrite (scoped out as a
  non-goal).
...
`scripts/deploy-global.ps1` (`npm run deploy`). The global `opencode.jsonc` and
...
`scripts/deploy-global.ps1` refuses to deploy an unsafe `tools/*.js`.
