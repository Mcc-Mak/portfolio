...
db.all(`SELECT defect_density, timestamp FROM builds ORDER BY id ASC`, (err, rows) => {
  ...
  // Mermaid xychart: actual densities plus the 5-build-ahead prediction point.
  const actualLine = data.map(d => d[1].toFixed(4)).join(', ');
  const yMax = Math.ceil(Math.max(GOAL, predicted, ...data.map(d => d[1])) * 2);
  const predLine = new Array(data.length).fill(0).join(', ') + ', ' + predicted.toFixed(4);
  const chartData = '```mermaid\nxychart-beta\n    title "Defect Density Trend with 5-Build Prediction"\n    x-axis "Build Number"\n    y-axis "Defect Density" 0..' +
    yMax + '\n    line [' + actualLine + '] "Actual"\n    line [' + predLine + '] "Predicted 5 ahead"\n```\n';
