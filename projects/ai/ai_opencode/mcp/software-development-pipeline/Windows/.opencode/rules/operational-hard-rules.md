...
single source of truth `C:\OpenCode\software-development-pipeline\Windows\`:
...
| Pipeline scripts (opencode-pipeline.ps1, collect-metrics.js, spc-control.js, predict-readiness.js, deploy-global.ps1) | `scripts/` | `~/.config/opencode/scripts/` |
...
2. Run `powershell -ExecutionPolicy Bypass -File scripts\deploy-global.ps1`
   (or `npm run deploy`) after EVERY change to any of the resources above.
...
8. **`tools/*.js` MUST be import-safe.** opencode auto-imports every `.js` in
   ...
   `@opencode-ai/plugin`. `scripts/deploy-global.ps1` refuses to deploy a
