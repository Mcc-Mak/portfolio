...
3. Copy __.\\melody_sheet\\__, __.\\music_sheet\\__, __.\\output_audio\\__, __.\\\*.py__ and __.\\requirements.txt__ from __<DIR_DEPLOYMENT>\\__ into __<DIR_PROJECT\\__
...
8. (If available) Remove the directory of `.\music_sheet\高唱入雲\`
  - format: `.\music_sheet\<SONG_NAME>\`
## SOP for Song Production
### Generate music sheet(s)
1. Change the working directory to __<DIR_PROJECT>__
2. Prepare `.\melody_sheet\高唱入雲.csv` as the melody file (format: `.\melody_sheet\<SONG_NAME>.csv`)
3. Prepare `.\music_sheet_highly_customized\高唱入雲\*.csv` for the arrangements of tailor-made musical instrument(s)
4. Run `.\Scripts\python.exe app.py 高唱入雲 "1_5@2_5@4_4@5_2@6_1@6_2@6_3@7_1"` (format: `.\Scripts\python.exe app.py "<SONG_NAME>" "<TRACK_i>_<CHANNEL_j>@..."`, where __<TRACK_i>__\___<CHANNEL_j>__ pattern is repeatable (separated by __@__))
### Generate "MIDI" file
1. Change the working directory to __<DIR_PROJECT>__
2. Fine-tune the notes in __.\\music_sheet\\<SONG_NAME>__
...
- Configuration File
  - __.\config.py__
    ...
    - **music_sheet_shorthand (effective to the product by `.\Scripts\python.exe app.py "<SONG_NAME>" "<TRACK_i>_<CHANNEL_j>@..."`)
- Music Sheet
  - __melody_sheet\\<SONG_NAME>.csv__ (effective to the product of __music_sheet\\<SONG_NAME>\\2_1.csv__ by `.\Scripts\python.exe app.py "<SONG_NAME>" "<TRACK_i>_<CHANNEL_j>@..."`)
  - __music_sheet\\<SONG_NAME>\\<Track_1-indexed>_<Channel_1-indexed>.csv__ (effective to the product of __.\\output_audio\\<SONG_NAME>.midi__)
### for Music Sheet
...
## Coding Standards
- **Naming:** Directories and files use `snake_case` ASCII (`[a-z0-9_.]` charset, no uppercase, no hyphens, no spaces). Chinese characters are preserved for natively-Chinese song directories and content files (e.g. `願你國度降臨`).
- **Python style:** PEP 8 conventions — `snake_case` for functions and variables, 4-space indentation, one import per line, spaces around operators.
- **No dead code:** Commented-out code blocks are removed. Debug `print` statements are not left in production source.
- **No comments:** Code is self-documenting; no inline comments are added unless explicitly requested.
- **Virtual environment:** The Python venv (`Lib/`, `Scripts/`, `pyvenv.cfg`, `__pycache__/`) is excluded from version control via `.gitignore`. Each developer creates their own venv following the Installation steps above.
- **Mutable defaults:** Functions avoid mutable default arguments (e.g. `jdata=None` instead of `jdata={}`).
- **None comparisons:** Use `is None` / `is not None` instead of `== None` / `!= None`.
- **Type checks:** Use `isinstance()` instead of `type() is`.
