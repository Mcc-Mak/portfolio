...
const request = async () => {
    ...
    async function addBooking(data) {
        ...
        if (response == 1) {
            await Swal.fire({
                icon: 'success',
                html: '<h4>Notification</h4><br><p><strong>Booking appointment added successfully!</strong></p><br><hr>Copyright &copy; 2022',
            ...
            await Swal.fire({
                icon: 'warning',
                html: '<h4>Warning</h4><br><p><strong>Failed to add the booking appointment. Please try again.</strong></p><br><hr>Copyright &copy; 2022',
                confirmButtonText: 'OK',
                allowOutsideClick: false
            });
        }
    ...
    const url = {
        "data": '/api/booking/query-data?usage=add',
        "nameEngOrg": '/api/booking/getDataList?attr=org&usage=add',
        "nameEngRoom": '/api/booking/getDataList?attr=room&usage=add',
        "nameEngUser": '/api/booking/getDataList?attr=user&usage=add',
        "dateStart": '/api/booking/getDataList?attr=dateStart&usage=add',
        "timeStart": '/api/booking/getDataList?attr=timeStart&usage=add',
        "dateEnd": '/api/booking/getDataList?attr=dateEnd&usage=add',
        "timeEnd": '/api/booking/getDataList?attr=timeEnd&usage=add',
    };
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (cell) {
                    ...
                    if (data.nameEngUser == "") {
                        return '<img src="/image/PENCIL.PNG" width="35px" height="35px" class="addBooking"/>';
                    } else {
                        return '';
                    }
                },
                cellClick: function (e, cell) {
                    let data = cell.getRow().getData();
                    if (data.nameEngUser == "") {
                        const req = async () => { await addBooking(data); }
                        req();
                    } else {
                        // [Notification]
                        Swal.fire({
                            icon: 'warning',
                            html: '<h4>Warning</h4><br><p><strong>This timeslot has been reserved by another user...</strong></p><br><hr>Copyright &copy; 2022',
                            confirmButtonText: 'OK',
                            allowOutsideClick: false
                        });
                    }
